...
# 安装所需的模块
install_module("pyaes")
install_module("requests")
install_module("curl_cffi")
...
def check_anti_cc() -> dict:
    ...
    res = cffi_requests.get(home_page, headers=headers)
...
def get_points(s: req_Session, number_c: int):
    if check_login_status(s, number_c):
        print_current_points(s)  # 打印帐户当前积分（执行之前）
        ...
        for i in range(len(url_list)):
            ...
            try:
                ...
                print("链接访问异常：" + str(e))
            continue
        print_current_points(s)  # 再次打印帐户当前积分（执行之后）
    else:
        print("请检查你的帐户是否正确！")
...
if __name__ == "__main__":
    ...
    if not username or not password:
        ...
        ip_info = print_my_ip()
        message_content = ip_info + "\n\n"
        message_content += "共检测到 " + str(len(user_list)) + " 个帐户，开始获取积分\n"
        message_content += "*" * 30 + "\n"
        ...
        for i in range(len(user_list)):
            try:
                ...
                get_points(s, i + 1)
                message_content += "第 " + str(i + 1) + " 个帐户获取积分完成\n"
                message_content += "*" * 30 + "\n"
            except Exception as e:
                message_content += "程序执行异常：" + str(e) + "\n"
                message_content += "*" * 30 + "\n"
            continue

        message_content += "程序执行完毕，获取积分过程结束"
        send_message("loc签到结果反馈", message_content)